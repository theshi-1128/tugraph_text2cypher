...
with open("temp_template_yago.json","r",encoding="utf-8") as f_test:
...
extracted_schema = [extracted_schema3]
...
with tqdm(total = 33) as pbar:
    for id, schema in enumerate(extracted_schema):
        ...
        for item in train_data:
            ...
            with open(f"temp_tempate2pairs_yago.json", "a", encoding="utf-8") as f7:
                json_output = json.dumps(output, ensure_ascii=False)  # 转换为 JSON 字符串，保留非 ASCII 字符
                f7.write(json_output)
                f7.write("\n")
                pbar.update(1)
