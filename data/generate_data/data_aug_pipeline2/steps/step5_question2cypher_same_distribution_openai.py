    ...
    while True:
        try:
            response = client.chat.completions.create(
                model="gpt-4o",  # 指定模型名称
                messages=messages,
                max_tokens=4000,  # 设置最大返回 token 数量
                n=1,  # 生成一个响应
                stop=None,  # 设置停止条件
...
# 文件夹路径
folder_path = './gen_questions/yago_new_schema/same_distribution_as_dataset'
output_folder = './gen_cyphers/yago_new_schema/same_distribution_as_dataset'
...
with tqdm(total=len(os.listdir(folder_path))) as pbar:
    ...
    for filename in os.listdir(folder_path):
        ...
        if "test" in filename:
            output_file_path = f"{output_folder}/test/step5_question2cypher_res_{num}_{db_name}.txt"
        else:
            output_file_path = f"{output_folder}/train/step5_question2cypher_res_{num}_{db_name}.txt"
